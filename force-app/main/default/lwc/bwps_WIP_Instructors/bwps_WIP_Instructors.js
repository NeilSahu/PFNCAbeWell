...
"Yoga":"data:image/svg+xml,%3Csvg id='Yoga' xmlns='http://www.w3.org/2000/svg' width='48' height='48' viewBox='0 0 48 48'%3E%3Crect id='Bounding_Box' data-name='Bounding Box' width='48' height='48' fill='none'/%3E%3Cg id='Group_8091' data-name='Group 8091' transform='translate(5 4)'%3E%3Cg id='Group_7158' data-name='Group 7158' transform='translate(0 0)'%3E%3Ccircle id='Ellipse_81' data-name='Ellipse 81' cx='5.201' cy='5.201' r='5.201' transform='translate(13.877 0)' fill='%23008ba7'/%3E%3Cpath id='Path_10971' data-name='Path 10971' d='M21.734,29.293h2.4a3.539,3.539,0,0,1,3.3,2l4.1,7.4c.3.5.6,1.2,1,1.8a12.223,12.223,0,0,0,3.2,3.6l2.6,1.9s2.1,1.6,1.4,3c-.8,1.5-3.1.8-6.3-1.1-4.6-2.8-7.8-11.1-7.8-11.1V50.3h-9.4v-13.5s-3.2,8.3-7.8,11.1c-3.2,1.8-5.5,2.5-6.3,1.1-.8-1.5,1.4-3,1.4-3l2.6-1.9a12.219,12.219,0,0,0,3.2-3.6c.4-.6.7-1.3,1-1.8l4.1-7.4a3.834,3.834,0,0,1,3.3-2h4Z' transform='translate(-1.957 -17.692)' fill='%23008ba7'/%3E%3Cpath id='Path_10972' data-name='Path 10972' d='M31.709,83.548a134.754,134.754,0,0,1-22-.5,3.008,3.008,0,0,0-3.3,2.6h0a3.013,3.013,0,0,0,2.7,3.3,141.539,141.539,0,0,0,29.1,0,3.013,3.013,0,0,0,2.7-3.3h0a3.008,3.008,0,0,0-3.3-2.6C35.61,83.248,33.709,83.448,31.709,83.548Z' transform='translate(-4.631 -50.144)' fill='%23008ba7'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E%0A"
  ...
  @track noFilterRecords = false;

  get getAGuestPassButton(){
    if(Id){
      return false;
    }
    return true;
      ...
      .then(result => {
        ...
        for (let i = 0; i < this.instemp.length; i++) {
          ...
          const ins = {
            ...
            exp: (this.instemp[i].Contact.BWPS_Type__c == "" ||this.instemp[i].Contact.BWPS_Type__c == undefined || this.instemp[i].Contact.BWPS_Type__c == null)  ? "" : String(this.instemp[i].Contact.BWPS_Type__c).split(";").join(", "),
            
            imgUrl: (this.instemp[i]?.Contact.BWPS_publicProfileLink__c) ? this.instemp[i].Contact.BWPS_publicProfileLink__c : this.instemp[i].FullPhotoUrl,
          }
          this.instructors.push(ins);
        }
        if (this.instructors.length <= 12) {
  ...
  searchfunction() {
    let allFilters = this.template.querySelectorAll('.filter');

    allFilters.forEach(elem => {
      if(elem.classList.contains('selected-filter'))
        elem.classList.remove('selected-filter');
    })
    this.filterToggle = '';
  ...
  handleAction(event) {
    ...
    this.noFilterRecords = false;
    // this.selectedflt = event.target.dataset.expertise;
    let allFilters = this.template.querySelectorAll('.filter');

    allFilters.forEach(elem => {
      let flag = false;
      if(elem.dataset.expertise == expertise && !elem.classList.contains('selected-filter')) flag = true;
      elem.classList.remove('selected-filter');
      if(flag) {
        elem.classList.add('selected-filter');
      }
    })
    console.log('expertise ',expertise);
    
    console.log('testing1 : ');
    console.log('allData : ',JSON.stringify(this.allData.tranners));
    console.log('filterToggle : ',this.filterToggle);
    try{
      if (this.filterToggle != expertise) {
        ...
        this.allData.tranners.forEach((element, index) => {
          ...
          if (String(element.exp).includes(expertise)) {
            this.checkData.push(element);
            //this.paginationData.push(element);
          }
          this.totalFilteredData = this.checkData.length;
        })
        //this.paginationFilteredData = this.checkData.slice(0, this.pageSize);
        ...
        // this.checkData.forEach(currentItem => {

        //   console.log('itemm---',JSON.stringify(currentItem));
        //   //TODO : currentItem
        // });
        console.log('paginationData : ',JSON.stringify(this.checkData));
        console.log('paginationFilteredData : ',JSON.stringify(this.paginationFilteredData));
      }
      else {
        this.value = false;
        this.noFilterRecords = false;
        ...
        this.checkData = [];
        this.paginationFilteredData = [];
        console.log('alldataTrainers2'+JSON.stringify(this.allData))

        console.log('alldataTrainers'+JSON.stringify(this.allData.tranners))
        this.allData.tranners.forEach((element) => {
          // if(element.exp.includes('Yoga')){
          ...
          // }
          this.checkData.push(element);
          // console.log('exp---- : ',element.exp+' - '+JSON.stringify(element));
          // if(i!=1&&i!=2&&i!=4) {
          //   element.exp='Yoga';
          //   this.checkData.push(element);
          // }
        ...
        if (this.checkData.length > 0) {
          ...
          //this.paginationFilteredData = this.checkData.slice(0, this.pageSize);
          this.paginationFilteredData = this.checkData;
