...
import INSTRUCTOR_DETAILS_IMAGE from '@salesforce/resourceUrl/WebsiteGeneralFiles';
import getIsntructorandClasses from '@salesforce/apex/BWPS_getInstructorsData.getIsntructorandClasses';
import getuserProfile from '@salesforce/apex/BWPS_getInstructorsData.GetuserProfile';
...
export default class Bwps_WIP_InstructorDetailView extends LightningElement {
    @track favIcon = `${allIcons}/PNG/Favorite.png `;
    @track unFavIcon = `${allIcons}/PNG/unfavorite.png `;
    inst_photoUrl = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/photo.png`;
    exerciseIconUrl = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/exercise.png`;
    like_icon = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/like.png`;
    share_icon = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/shareIcon.png`;
    low_seated = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/1.png`;
    medium_seated = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/2.png`;
    high_seated = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/3.png`;
    duration_15min = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/15.png`;
    duration_30min = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/30.png`;
    duration_45min = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/45.png`;
    duration_60min = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/60.png`;
    d_class_photo = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/d_class_photo.png`;
    inst_profile_icon = `${INSTRUCTOR_DETAILS_IMAGE}/WebsiteGeneralFiles/inst_profile_icon.png`;
    ...
    @track instructorExpIcon;
    connectedCallback() {
        console.log('Instructor ID : parent>>>>>>>>>>>', this.instructor);
        console.log('Instructor name : parent>>>>>>>>>>>', this.instructorname);
        console.log('Instructor profile : parent>>>>>>>>>>>', this.instructorprofile);
        console.log('Instructor exp : parent>>>>>>>>>>>', this.instructorexp);
        if(this.instructorprofile == undefined || this.instructorprofile == ''){
            this.instructorprofile = this.ProfilePhoto;
        }
        if(this.instructorexp != null && this.instructorexp != ''){
            let exp;
            if(this.instructorexp.includes(',')){
                exp = this.instructorexp.split(',')[0].toLowerCase();
            }else{
                exp = this.instructorexp.toLowerCase();
            }

            if(this.img[exp] != undefined && this.img[exp] != '' && this.img[exp] != null){
                this.instructorExpIcon = this.img[exp];
            }else{
                this.instructorExpIcon = this.img['yoga'];
            }
        }
        else{
            this.instructorExpIcon = false;
        }
        allEntitySubs()
            .then(result => {
                ...
                if (result) {
                    ...
                    result.forEach(es => {
                        if (es.ParentId == this.instructor) {
                            this.isFav = true;
                        }
                    });
                }
        ...
        getIsntructorandClasses({ InstructorId: this.instructor })
            .then(result => {
                ...
                try{
                    for (i = 0; i < result.length; i++) {
                        console.log('resi.enroll.length ', result[i].Enrolled_Classes__r);
                        var enrolledUsers;
                        if (result[i].Enrolled_Classes__r != undefined) {
                            enrolledUsers = result[i].Enrolled_Classes__r.length;
                        }
                        else {
                            enrolledUsers = 0;
                        }

                        var minutesfinal = (result[i].BWPS_EndTime__c - result[i].BWPS_StartTime__c) / (1000 * 60);
                        ...
                        if (result[i].Integrity__c == 'High/Active') {
                            intensityLogo = highSignal;
                        }
                        else if (result[i].Integrity__c == 'Medium') {
                            intensityLogo = mediumSignal;
                        }
                        else if (result[i].Integrity__c == 'Low/Seated') {
                            intensityLogo = lowSignal;
                        }
                        ...
                        let classDay = new Date();
                        classDay.setDate(result[i].BWPS_Date__c.split('-')[2]);
                        classDay.setMonth(result[i].BWPS_Date__c.split('-')[1]);
                        classDay.setFullYear(result[i].BWPS_Date__c.split('-')[0]);

                        classDay = this.weekdays[classDay.getDay()];

                        let classTiming = result[i].BWPS_StartTime__c;

                        if (result[i].BWPS_StartTime__c != undefined) {
                            var timeInHours = ((Number(result[i].BWPS_StartTime__c) / 1000) / 60) / 60;
                            if (timeInHours == 0) {
                                classTiming = '12:00 AM';
                            } else {
                                var isInteger = Number.isInteger(timeInHours)
                                let timeOfDay = timeInHours < 12 ? 'AM' : 'PM';
                                timeInHours -= timeInHours <= 12 ? 0 : 12;
                                if (isInteger) {
                                    classTiming = String(timeInHours).padStart(2, '0') + ':00 ' + timeOfDay;
                                } else {
                                    ...
                                    classTiming = String(hours).padStart(2, '0') + ':' + String(min).padStart(2, '0') + ' ' + timeOfDay;
                                }
                            }
                        }
                        else {
                            result[i].BWPS_StartTime__c = 0;
                            classTiming = '00:00 AM';
                        }

                        let fav = this.allSubEntityMap.hasOwnProperty(result[i].Id);
                        // if(result[i].Schedule_Type__c=='Live' || result[i].Schedule_Type__c=='Hybrid'){
                        console.log('result[i].Schedule_Type__c ', result[i].Schedule_Type__c);
                        obj = {
                            "id": result[i].Id,
                            "classDay" : classDay,
                            "class_timimg": classTiming,
                            "inst_name": result[i].Name,
                            "seating_capacity_text": result[i].Integrity__c,
                            "activity_description": result[i].BWPS_Description__c,
                            "members": enrolledUsers,
                            "exerciseIconUrl": this.exerciseIconUrl,
                            "inst_shareUrl": this.share_icon,
                            "inst_likeUrl": fav?this.favIcon:this.unFavIcon,
                            "favClassStatus": fav,
                            "inst_profile_icon": this.inst_profile_icon,
                            "activity_name": result[i].Name,
                            "activity_description": result[i].BWPS_Description__c,
                            "d_class_photo": this.d_class_photo,
                            "seating_status_icon": intensityLogo,
                            "seating_capacity_icon": intensityLogo,
                            "duration_icon": timeLogo,
                            "duration_text": minutesfinal,
                            "d_class_name": result[i].Name,
                            "d_class_description": result[i].BWPS_Description__c,
                        }
                        arr.push(obj);

                        //  }
                        //if(result[i].Schedule_Type__c=='In Person' || result[i].Schedule_Type__c=='Hybrid'){
                        console.log('result[i].Schedule_Type__c ', result[i].Schedule_Type__c);
                        objDemand = {
                            "id": result[i].Id,
                            "class_timimg": minutesfinal,
                            "inst_name": result[i].Name,
                            "seating_capacity_text": result[i].Integrity__c,
                            "activity_description": result[i].BWPS_Description__c,
                            "members": enrolledUsers,
                    ...
                    console.log('arr ', arr);
                    this.scheduleClass = arr;
                    this.scheduleClassDemand = arrDemand;
                }
                catch(error){
                    console.log('errrr : ',error.message, error, JSON.stringify(error));
                }
        ...
        getuserProfile({ InstructorId: this.instructor })
            .then(result => {
                console.log('profile photo result ', result)
                this.ProfilePhoto = result.MediumPhotoUrl;
    ...
    inst_detail = {
        "inst_photoUrl": this.inst_photoUrl,
        "inst_profile_icon": this.inst_profile_icon,
        "inst_name": "Christy Erwin",
        "exerciseIconUrl": this.exerciseIconUrl,
        "inst_type": "Exercise Instructor",
        "inst_shareUrl": this.share_icon,
        "inst_likeUrl": this.like_icon,
        "about_inst": "Christy Erwin Nobis asperiores et cumque aliquam aliquid ipsum, ea a provident modi laborum totam aspernatur amet!Odit voluptate enim ullam. Enim voluptate blanditiis omnis nihil ea dolorum dolores harum,reiciendis necessitatibus eum rerum beatae, quod commodi veritatis amet!",

        "live_classes": [
            {
                "id": 0,
                "class_timimg": "TUESDAYS AT 10 AM EST",
                "activity_name": "DANCE FOR PARKINSON'S",
                "activity_description": "Made possible with support from Virginian Outpatient Therapy",
                "seating_capacity_icon": this.high_seated,
                "seating_capacity_text": "High/Seated",
                "duration_icon": this.duration_15min,
        ...
        "classes_on_demand": [
            {
                "id": 0,
                "d_class_photo": this.d_class_photo,
                "seating_status_icon": this.high_seated,
                "d_class_name": "HIGH AEROBIC EXERCISE FOR PARKINSON'S",
                "d_class_description": "Made possible with support from Lorien Encore",
    ...
    backClickedHandle() {
        //window.history.back();
        this.dispatchEvent(new CustomEvent('instructorreturn', {
            "detail": this.offinstructor,
            bubbles: true,
            composed: true
        }));
    }

    instShareClickHandler() {

    }
    instLikeClickHandler(event) {
        this.loading = true;
        console.log('inst_Id : ', event.target.dataset.id);

        if (this.VALID_USER) {
            let classId = event.target.dataset.id;
            //let status = event.target.dataset.status;
            // imperative apex call
            follow({ recId: classId, isFollowing: this.isFav })
            .then(result => {
                console.log('response : ', result);
                //let scheduleCls = this.scheduleClass.find(sc => sc.id == event.target.dataset.id);
                if (result == true) {
                    this.isFav = true;
                    //scheduleCls.inst_likeUrl = this.favIcon;
                    this.template.querySelector('c-toast-message').showToast('success', 'Favorite successfully.');
                    this.loading = false;
                }
                else if (result == false) {
                    this.isFav = false;
                    //scheduleCls.inst_likeUrl = this.unFavIcon;
    ...
    scLikeClickHandler(event) {
        ...
        if (this.VALID_USER) {
            ...
            let status = event.target.dataset.status;
            // imperative apex call
            follow({ recId: classId, isFollowing: status })
                .then(result => {
                    console.log('response : ', result);
                    let scheduleCls = this.scheduleClass.find(sc => sc.id == event.target.dataset.id);
                    if (result == true) {
    ...
    renderedCallback() {
        window.scrollTo({ top: 0, left: 0, behaviur: 'smooth' })
    }
}